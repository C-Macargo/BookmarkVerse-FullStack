...
import { createLogin, createUser, registerValidBody } from '../factories/userFactories';
...
describe('REGISTER /auth/register', () => {
  ...
  describe('when body is valid', () => {
    it('should register a user and respond with status 201', async () => {
      await createUser();
      const response = await api.post('/auth/register').send(registerValidBody);
...
describe('LOGIN /auth/login', () => {
  ...
  describe('when body is valid', () => {
    it('should respond with status 200 with user Data and a token', async () => {
      const loginBody = {
